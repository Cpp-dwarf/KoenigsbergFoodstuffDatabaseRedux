queue()
  .defer(d3.csv, "/files/theme/BarGraph/data/fish_by_pfund_transposed.csv")
...
function ready(error, data, empty) {
  ...
  colors.domain(d3.keys(data[0]).filter(function(key) { return key !== "date" && key !== "id"; }));
...
function tooltipTimeline(d) {
  ...
  "<table>" +
  "<tr>" +
  "<td><strong>Visit percentage</strong>: " + d3.format("%")(d.value) + "</td>" +
  "</tr>" +
  "</table>";
}
