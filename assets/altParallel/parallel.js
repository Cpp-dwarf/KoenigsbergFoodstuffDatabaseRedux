...
//modified to round floats
  cells = rows.selectAll("td")
    .data(function(row) {
        return columns.map(function(column) {
			if(row[column] % 1 !==0 && typeof(row[column]) == "number")
			{
				var value=row[column].toFixed(2);
...
d3.csv("/files/theme/altParallel/data/visit_percentage.csv", type, function(error, timechart) {
  ...
  .enter().append("rect")
    ...
    .style("fill", "steelblue");
		...
		//bad way to do timeline percentages but time is low and d3 is misbehaving
	var offset=(width/32)-20; //handles dynamic spacing for percents
	
	var percents=["92%","0%","92%","33%","0%","75%","96%","0%","0%","74%","0%","0%","32%","0%","0%","0%","0%","0%","0%","0%","0%","0%","6%","0%","0%","0%","0%","0%","0%","0%","0%","0%"]; //data
